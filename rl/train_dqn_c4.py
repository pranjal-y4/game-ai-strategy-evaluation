...
rl/train_dqn_c4.py
────────────────────────────────────────────────────────────────────────────
Train a Deep Q-Network (DQN) on the full 6×7 Connect 4 board.

DQN handles the large state space (42 features) via function approximation,
making board reduction unnecessary.
Network: 42 → 128 → 64 → 7

Role alternation (enabled by default):
  Connect 4 has a substantial first-mover advantage.  Training in both
  roles produces a more robust policy.

Generalization evaluation:
  At each checkpoint the agent is evaluated against both a random and a
  semi-smart opponent so the training curve shows generalization.

Usage:
    python rl/train_dqn_c4.py --episodes 100000 --seed 42
    python rl/train_dqn_c4.py --episodes 100000 --opponent semi --seed 42
"""
import sys, os
...
def train(episodes: int, seed: int, model_path: str, opponent: str = "random",
          ...
          decay_steps: int = 50_000, eval_every: int = 2_500):

    set_seed(seed)
    env   = Connect4Env(rows=6, cols=7, opponent=opponent)
    ...
    eval_random_env = Connect4Env(rows=6, cols=7, opponent="random")
    eval_semi_env   = Connect4Env(rows=6, cols=7, opponent="semi")

    # ── CSV logging ───────────────────────────────────────────────────────────
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = os.path.join(RESULTS_DIR, f"rl_training_metrics_c4_dqn_{ts}.csv")
    ...
    def callback(ep: int, win_rate: float, epsilon: float):
        ...
        # Greedy eval (150 games vs random, as P1)
        wins = draws = losses = 0
        saved = agent.epsilon
        agent.epsilon = 0.0
        for _ in range(150):
        ...
        # Role-split eval
        p1_wr, p2_wr = _greedy_eval(agent, eval_random_env, 75)

        # Generalization: eval vs semi opponent
        p1_semi, p2_semi = _greedy_eval(agent, eval_semi_env, 50)
    ...
    print(f"Training Connect4 DQN 6x7  ({episodes} episodes, opponent={opponent}, "
          f"role_alternation=True, seed={seed}) …")
    agent.train(n_episodes=episodes, eval_every=eval_every,
                eval_episodes=150, callback=callback)

    agent.save(model_path)
    final_wr = agent.evaluate(200)
    print(f"\n  Model saved → {model_path}")
    print(f"  Final win rate (greedy, 200 eps): {final_wr:.2%}")
...
def main():
    parser = argparse.ArgumentParser(description="Train DQN for full 6x7 Connect 4")
    ...
    parser.add_argument("--decay_steps", type=int,   default=50_000)
    parser.add_argument("--eval_every",  type=int,   default=2_500)
    parser.add_argument("--model_path",  default=os.path.join(MODEL_DIR, "c4_dqn.pkl"))
